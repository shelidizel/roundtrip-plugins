<?php 
   ...
   */
   ...
   $title = esc_html__('New Booking Archive Template', 'gowilds-themer');
?>
<div class="gva-template-layout">
   <h3><?php echo esc_html__('Booking Archive Templates', 'gowilds-themer') ?></h3>
   <div class="list-template-layout">
      ...
      <?php foreach ($this->get_templates($type) as $item) { ?>
         ...
         <div class="item" data-type="<?php echo esc_attr($type) ?>">
            <div class="state">
              <a class="btn-set-config-state <?php echo esc_attr($state) ?>" href="#" title="<?php echo esc_attr('Active Default', 'gowilds-theme') ?>" data-id="<?php echo $item['id'] ?>" data-type="<?php echo esc_attr($type) ?>"><i class="dashicons-before dashicons-marker"></i><span class="text">Default</span></a>
            </div>
