...
if ( ! class_exists( 'Jet_Search_Ajax_Handlers' ) ) {
	...
	class Jet_Search_Ajax_Handlers {
		...
		public function set_posts_search( $query ) {
		...
		*/
		...
		public function cf_search_clauses( $args, $query ) {
			...
			if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
				...
				$args['join'] .= " AND {$wpdb->posts}.ID = wpml_translations.element_id AND wpml_translations.language_code='{$current_lang}'";
			}
			...
			if ( ( $key = array_search( '_sku', $cf_keys ) ) !== false ) {
				if ( function_exists( 'WC' ) ) {
					...
					if ( false != $search_query_param ) {
						$query->query['s'] = $search_query_param;
					}
