...
jQuery(document).ready(function () {
	...
	function recentVisiroeByIPTable()
	{
		jQuery('#recent_visit_by_ip').DataTable({
			...
			ajax: ahc_ajax.ajax_url+'?action=recent_visitor_by_ip&year='+jQuery("#year").find(":selected").val()+"&month="+jQuery("#month").find(":selected").val()+"&ip="+jQuery("#ip_addr").val(),
			dataSrc: 'data',
			columns: [
				{ data: 'hit_ip_address' },
				{ data: 'ctr_name' },
			...
			buttons: [{
				...
				action: function (e, dt, node, config) {
					...
					jQuery.ajax({
					url:  ahc_ajax.ajax_url+'?action=recent_visitor_by_ip&page=all&year='+jQuery("#year").find(":selected").val()+"&month="+jQuery("#month").find(":selected").val()+"&ip="+jQuery("#ip_addr").val(),
					data: dt.ajax.params(),
					success: function(res, status, xhr) {
...
var columnsToExport = ["hit_ip_address", "ctr_name", "time", "day_hits"];

// Extract values for the specified columns
var values = parsedData.data.map(function(item) {
...
// Create a worksheet for the exported data
var worksheet = XLSX.utils.aoa_to_sheet([columnsToExport].concat(values));
...
jQuery("#recent_visit_by_ip_processing").hide();
